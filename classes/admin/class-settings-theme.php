...
class Settings_Theme {
	...
	/**
	 * Will return true if this is the LSX Videos settings page.
	 *
	 * @var array
	 */
	...
	public function generate_navigation( $cmb_id, $object_id, $object_type, $cmb2_obj ) {
		if ( 'lsx_videos_settings' === $cmb_id && 'lsx_videos_options' === $object_id && 'options-page' === $object_type ) {
	...
	public function navigation_js( $cmb_id, $object_id, $object_type, $cmb2_obj ) {
		if ( 'lsx_videos_settings' === $cmb_id && 'lsx_videos_options' === $object_id && 'options-page' === $object_type ) {
			?>
			<script>
				var LSX_VIDEOS_CMB2 = Object.create( null );

				;( function( $, window, document, undefined ) {

					'use strict';

					LSX_VIDEOS_CMB2.document = $(document);

					/**
					 * Start the JS Class
					 */
					LSX_VIDEOS_CMB2.init = function() {
						var tab = LSX_VIDEOS_CMB2.urlParam( 'cmb_tab' );
						if ( 0 === tab || '0' === tab ) {
							tab = '';
						}
						LSX_VIDEOS_CMB2.addTabInput( tab );
						LSX_VIDEOS_CMB2.prepNavigation( tab );
						LSX_VIDEOS_CMB2.watchNavigation();
					};

					LSX_VIDEOS_CMB2.addTabInput = function( tab = '' ) {
						var counter = 1;
						$( "form.cmb-form" ).append('<input type="hidden" name="cmb_tab" value="' + tab + '" />');
					}

					LSX_VIDEOS_CMB2.prepNavigation = function( tab = '' ) {
						var counter = 1;
					...
					LSX_VIDEOS_CMB2.watchNavigation = function() {
					...
					LSX_VIDEOS_CMB2.urlParam = function(name){
					...
					LSX_VIDEOS_CMB2.document.ready( function() {
						LSX_VIDEOS_CMB2.init();
					} );
